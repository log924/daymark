...
export default function Home() {
  ...
  const visibleArticles =
    active === "Saved" ? savedArticles : active === "Sources" ? articles : briefArticles;
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav aria-label="Main navigation">
          {["Brief", "Latest", "Sources", "Saved", "Settings"].map((name) => (
            <button key={name} className={active === name ? "nav-item active" : "nav-item"} onClick={() => { setActive(name); if (name === "Latest" && !selectedLatestSourceId) setSelectedLatestSourceId(sources[0]?.id ?? null); }}>
      ...
      <section className="content" id="top">
        ...
        {active === "Latest" && <section className="latest-browser"><aside className="latest-sources"><div><p className="eyebrow">YOUR SOURCES</p><h2>Subscriptions</h2></div><div className="latest-source-list">{sources.map((source, index) => { const unread = articles.filter((article) => article.sourceId === source.id && !article.readAt).length; return <button key={source.id} className={selectedLatestSource?.id === source.id ? "latest-source active" : "latest-source"} onClick={() => setSelectedLatestSourceId(source.id)}><span className="source-logo" style={{ background: sourceColors[index % sourceColors.length] }}>{source.name.charAt(0).toUpperCase()}</span><span>{source.name}</span><b>{unread}</b></button>; })}</div></aside><div className="latest-feed"><section className="section-heading"><div><p className="eyebrow">LATEST UNREAD</p><h2>{selectedLatestSource ? selectedLatestSource.name : "Choose a source"}</h2></div>{latestUnreadArticles.length > 6 && <button className="text-button" onClick={() => setShowAll(!showAll)}>{showAll ? "Show less" : "See all"} <span>→</span></button>}</section>{selectedLatestSource && !latestUnreadArticles.length && <div className="empty-state">You’re all caught up with {selectedLatestSource.name}.</div>}<section className="story-grid">{latestUnreadArticles.slice(0, showAll ? undefined : 6).map((article, index) => <article className={`story-card ${colors[index % colors.length]} is-unread`} key={article.id} role="button" tabIndex={0} onClick={() => openArticle(article)} onKeyDown={(event) => event.key === "Enter" && openArticle(article)}><div className="story-meta"><span className="source-dot"/><span>{selectedLatestSource?.name}</span><em>·</em><span>{relativeTime(article.publishedAt ?? article.savedAt)}</span><span className="unread-badge">Unread</span><button onClick={(event) => { event.stopPropagation(); toggle(article.id, setSaved); }} aria-label="Save article">{saved.includes(article.id) || article.status === "saved" ? "♥" : "♡"}</button></div><span className="tag">{selectedLatestSource?.kind ?? "RSS"}</span><h3>{article.title}</h3><p>{articleSnippet(article)}</p><div className="story-foot"><span>✦ Fresh from this source</span><div className="story-actions"><button onClick={(event) => { event.stopPropagation(); processInsight(article); }} disabled={processingInsight === article.id}>{processingInsight === article.id ? "Summarizing…" : "Summarize"}</button><button className="read" onClick={(event) => { event.stopPropagation(); openArticle(article); }}>Open <span>→</span></button></div></div></article>)}</section></div></section>}

        {active !== "Settings" && active !== "Brief" && active !== "Latest" && <section className="section-heading"><div><p className="eyebrow">{active === "Saved" ? "SAVED" : "BY SOURCE"}</p><h2>{active === "Saved" ? "For later" : "All incoming pieces"}</h2></div><button className="text-button" onClick={() => setShowAll(!showAll)}>{showAll ? "Show less" : "See first 6"} <span>→</span></button></section>}

        {active !== "Settings" && active !== "Brief" && active !== "Latest" && <section className="story-grid">

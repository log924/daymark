  ...
  return (
    <main className="app-shell">
      ...
      <section className="content" id="top">
        ...
        {active === "Brief" && <section className="brief-library brief-library-single">
          <article className="daily-brief daily-edition">
            {dailyBrief ? <>
              ...
              <section className="daily-edition-index"><div className="daily-edition-index-head"><h3>今日看点</h3><span>{briefSections.reduce((count, section) => count + section.articles.length, 0)} 篇</span></div>{briefSections.map((section, index) => <a className="daily-edition-index-row" href={`#brief-section-${section.id}`} key={section.id}><b>{String(index + 1).padStart(2, "0")}</b><div><strong>{section.title}</strong><p>{section.articles[0]?.title}</p></div><span>{section.articles.length}</span></a>)}</section>
              ...
              <div className="daily-edition-sections">{briefSections.map((section, index) => <section className="daily-edition-section" id={`brief-section-${section.id}`} key={section.id}><header><b>{String(index + 1).padStart(2, "0")}</b><h3>{section.title}</h3><span>{section.articles.length} 篇</span></header><div className="reading-timeline daily-edition-timeline">{section.articles.map((article) => {
        ...
        {active !== "Settings" && active !== "Books" && active !== "Brief" && <section className="story-grid">
          {visibleArticles.map((article, index) => {
            ...
            active === "Brief" ? <article className={`timeline-story${article.readAt ? " is-read" : " is-unread"}`} key={article.id} role="button" tabIndex={0} onClick={() => openArticle(article)} onKeyDown={(event) => event.key === "Enter" && openArticle(article)}>
              ...
              <span className="tag">{source?.kind ?? "Manual"}</span><h3 title={article.title}>{article.title}</h3><p>{articleSnippet(article)}</p>
        ...
        {active !== "Settings" && active !== "Brief" && active !== "Books" && !visibleArticles.length && <div className="empty-state">{active === "Read" ? "Articles you finish will appear here." : active === "Passed" ? "Nothing has been passed yet. Passed articles stay here until you restore them to the queue." : "Refresh your connected Inoreader feeds, and Daymark will start filling this desk."}</div>}
      </section>

      {showCapture && (
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Add book">
          <div className="capture-modal">
            <div className="modal-head"><div><p className="eyebrow">ADD BOOK</p><h2>Bring a book into Daymark</h2></div><button onClick={() => setShowCapture(false)} aria-label="Close">×</button></div>
            <form className="book-add-form" onSubmit={addBook}><label><span>Book title</span><input value={bookTitle} onChange={(event) => setBookTitle(event.target.value)} placeholder="Title (optional if you paste a link)" autoFocus /></label><label><span>Goodreads or Douban link</span><input value={bookUrl} onChange={(event) => setBookUrl(event.target.value)} placeholder="https://..." /></label><label><span>Shelf</span><select value={bookStatus} onChange={(event) => setBookStatus(event.target.value as Book["status"])}><option value="to_read">To be read</option><option value="reading">Reading now</option><option value="read">Read</option></select></label><button disabled={busy || (!bookTitle.trim() && !bookUrl.trim())}>Add to shelf</button></form>
          </div>
        </div>
      )}

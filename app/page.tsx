      ...
      {selectedBook && (
        <div className="modal-backdrop reader-backdrop" role="dialog" aria-modal="true" aria-label="Book details" onClick={() => setSelectedBook(null)}>
          <article className="reader-modal book-reader-modal" onClick={(event) => event.stopPropagation()}>
            <div className="modal-head"><div><p className="eyebrow">YOUR LIBRARY · {selectedBook.status === "reading" ? "READING NOW" : selectedBook.status === "to_read" ? "TO BE READ" : "READ"}</p><h2>{selectedBook.title}</h2>{selectedBook.author && <p className="book-reader-author">by {selectedBook.author}</p>}</div><button onClick={() => setSelectedBook(null)} aria-label="Close">×</button></div>
            <section className="book-reader-hero">{selectedBook.coverUrl ? <img src={bookCoverSrc(selectedBook.coverUrl)} alt={`Cover of ${selectedBook.title}`} /> : <div className="book-reader-spine">{selectedBook.title.slice(0, 1)}</div>}<div className="book-facts"><div className="book-status-control"><span>Reading status</span><select aria-label={`Reading status for ${selectedBook.title}`} value={selectedBook.status} onChange={(event) => void updateBookStatus(selectedBook, event.target.value as Book["status"])}><option value="reading">Reading now</option><option value="to_read">To be read</option><option value="read">Read</option></select></div>{selectedBook.interestScore !== null && <div className="book-fit-large"><strong>{selectedBook.interestScore}</strong><span>interest fit</span></div>}<dl>{selectedBook.publishedYear && <><dt>Published</dt><dd>{selectedBook.publishedYear}</dd></>}{selectedBook.isbn && <><dt>ISBN</dt><dd>{selectedBook.isbn}</dd></>}{selectedBook.subjects && <><dt>Tags</dt><dd>{selectedBook.subjects}</dd></>}</dl></div></section>
            <section className="book-detail-section"><div className="detail-heading"><div><p className="eyebrow">ABOUT THE BOOK</p></div>{selectedBook.canonicalUrl?.includes("book.douban.com") && <button className="analyze-button" disabled={busy} onClick={() => void refreshBookMetadata(selectedBook)}>Refresh from Douban</button>}</div><div className="reader-body">{selectedBook.description ? <p>{selectedBook.description}</p> : <p>No description was available when this book was added. Use “Refresh from Douban” to import its latest book details.</p>}</div></section>

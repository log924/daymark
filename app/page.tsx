      ...
      {selectedBook && (
        <div className="modal-backdrop reader-backdrop" role="dialog" aria-modal="true" aria-label="Book details" onClick={() => setSelectedBook(null)}>
          <article className="reader-modal book-reader-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <section className="book-detail-section"><p className="eyebrow">ABOUT THE BOOK</p><div className="reader-body">{selectedBook.description ? <p>{selectedBook.description}</p> : <p>No description was available when this book was added. Its title, shelf, and reading connections are still saved here.</p>}</div></section>
